...
namespace Speed.Data.MetaData
{
    ...
    public class DbColumnInfo
    {
            ...
            : this()
        {
            ...
            //    if (tb.Columns[i].ColumnName != "ColumnName" && tb.Columns[i].DataType == typeof(string))
        ...
        {
            DbColumnInfo ret = new DbColumnInfo();
            // ret.TableCatalog = Conv.ToString(row["TABLE_CATALOG"]);
            // ret.TableSchema = Conv.ToString(row["TABLE_SCHEMA"]);
            ret.TableName = tableName;
            ret.ColumnName = col.ColumnName;
            ret.OrdinalPosition = col.Ordinal;
            ret.ColumnDefault = col.DefaultValue.ToString();
            ret.IsNullable = col.AllowDBNull;
            // ret.DataType = Conv.ToString(row["DATA_TYPE"]);
            // ret.CharacterMaximumLength = Conv.ToInt32(row["CHARACTER_MAXIMUM_LENGTH"]);
            // ret.CharacterOctetLength = Conv.ToInt32(row["CHARACTER_OCTET_LENGTH"]);
            // ret.NumericPrecision = Conv.ToByte(row["NUMERIC_PRECISION"]);
            // ret.NumericPrecisionRadix = Conv.ToInt16(row["NUMERIC_PRECISION_RADIX"]);
            // ret.NumericScale = Conv.ToInt32(row["NUMERIC_SCALE"]);
            // ret.DatetimePrecision = Conv.ToInt16(row["DATETIME_PRECISION"]);
            // ret.CharacterSetCatalog = Conv.ToString(row["CHARACTER_SET_CATALOG"]);
            // ret.CharacterSetSchema = Conv.ToString(row["CHARACTER_SET_SCHEMA"]);
            // ret.CharacterSetName = Conv.ToString(row["CHARACTER_SET_NAME"]);
            // ret.CollationCatalog = Convert.ToString(row["COLLATION_CATALOG"]);

            ret.IsIdentity = col.AutoIncrement;
        }
