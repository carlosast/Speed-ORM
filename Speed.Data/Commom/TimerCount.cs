...
namespace Speed
{
    ...
    internal class TimerCountInfo
    {
        ...
        {
            return FormatTotal(title, end.Subtract(start), start, end, maxWidth);
        }
        public static string FormatTotal(string title, TimeSpan ts, DateTime start, DateTime end, int maxWidth)
        {
            var total = Math.Round(ts.TotalSeconds, 6).ToString().PadRight(9);
            return title.PadRight(maxWidth) + "--> " + total + "s";
            //return title.PadRight(maxWidth) + "--> " + ts.ToString().PadRight(16) + "= " + total + "s";
            //return title.PadRight(maxWidth) + "--> Início: " + start.ToString("HH:mm:ss.ffff") + " - Fim: " + end.ToString("HH:mm:ss.ffff") + " - Tempo: " + ts.ToString().PadRight(16) + "= " + total + "s";
        }
    }
}
