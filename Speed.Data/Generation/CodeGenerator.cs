...
namespace Speed.Data.Generation
{
    ...
    public class CodeGenerator
    {
        ...
        static CodeGenerator()
        {
            ...
            // assemblies.Add(typeof(TimerCount).GetTypeInfo().Assembly.Location); // "Speed.Common.dll"
            //assemblies.Add(typeof(TimerCount).GetAssemblyPath());
        ...
        public static DataClass GenerateDataClass(Database db, Type type, out DbTableInfo table, out Dictionary<string, DbColumnInfo> infos)
        {
            ...
            DataClass dc = (DataClass)Activator.CreateInstance(objType, db.Provider);
            dc.Code = text;
        ...
        public static Assembly Compile(Database db, Type typeOfOneClass, string text, string id, string fileDll = null, Dictionary<string, string> otherUsings = null)
        {
            ...
            references.Add(db.GetType().Assembly.Location);
            references.Add(db.Provider.GetType().Assembly.Location);
            ...
            others.Add(Path.GetDirectoryName(typeof(object).Assembly.Location));
            foreach (var file in others)
                if (!references.Contains(file, StringComparer.OrdinalIgnoreCase))
                    references.Add(file);
                    */
            ...
            var mdReferences = references.Distinct(StringComparer.OrdinalIgnoreCase).Select(p => MetadataReference.CreateFromFile(p)).ToList();

            var xs = AssemblyHelper.CollectReferences();
            foreach (var x in xs)
                if (!mdReferences.Contains(x))
                    mdReferences.Add(MetadataReference.CreateFromFile(x.Display));
            ...
            using (var ms = new MemoryStream())
            {
                ...
                else
                {
                    ...
                    try
                    {
                        // se der erro deve continuar
                        if (fileDll != null && errText.Length == 0)
                        {
                            ms.Seek(0, SeekOrigin.Begin);
                            using (FileStream file = new FileStream(fileDll, FileMode.Create, System.IO.FileAccess.Write))
                                ms.CopyTo(file);
                        }
                    }
                    catch { }
