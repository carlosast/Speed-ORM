...
namespace Speed.Data.Generation
{
    ...
    public class CodeGenerator
    {
        ...
        public static string GenerateDataClassCode2(Database db, Type type, out string className, out string fullTableName, out DbTableInfo table, out Dictionary<string, DbColumnInfo> infos, Dictionary<string, string> otherUsings)
        {
            className = "DataClass" + type.Name;
            string fullTypeName = type.Namespace + "." + type.Name;
            ...
            foreach (KeyValuePair<string, DbColumnInfo> pair in table.Columns)
            {
                ...
                if (classColumns.ContainsKey(pair.Key))
                {
                    ...
                    if (prop == "byte[]")
                        ...
                        methodName = "GetDecimal(dr, [ColumnName])";
                    else if (prop == "float")
                        methodName = "GetFloat(dr, [ColumnName])";
                    else if (prop == "guid")
                        methodName = "GetGuid(dr, [ColumnName])";
                    else if (prop == "int32")
                        methodName = "GetInt32(dr, [ColumnName])";
                    else if (prop == "int64")
                        methodName = "GetInt64(dr, [ColumnName])";
                        ...
                        methodName = "(SByte)GetByte(dr, [ColumnName])";
                    else if (prop.Contains("sqlhierarchyid"))
                    {
                        // TODO: checar se é necessário
                        //if (!otherUsings.ContainsKey("using Microsoft.SqlServer.Types;"))
                        //{
            ...
            text = text.Replace("[UpdateColumns]", Q(db, updateColumns));
            text = text.Replace("[UpdateColumns]", Q(db, updateColumns));
            text = text.Replace("[SortDefault]", Q(db, !string.IsNullOrWhiteSpace(pkColumns) ? pkColumns : db.Provider.GetObjectName(table.Columns.First().Key)));
