    ...
    internal class DataClassCache
    {
        ...
        */
        ...
        private void GenerateDataClasses(Database db, List<Type> types, bool singleClass = false)
        {
            ...
            TimerCount tc = new TimerCount("GenerateDataClasses");
            ...
            foreach (var type in types)
            {
                ...
                //if (!otherUsings.ContainsKey("System.Runtime.dll"))
                //    otherUsings.Add("using System.Runtime", typeof(System.Runtime.CompilerServices.ContractHelper).Assembly.Location);
            ...
            if (hasCache)
            {
                try
                {
                    string dllPath = types[0].Assembly.Location;
                    string dir = Path.Combine(
                        Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Speed",
                        Speed.IO.FileTools.ToValidPath((Cryptography.Hash(dllPath.ToUpperInvariant() + "|" + db.Connection.ConnectionString.ToUpperInvariant()).Replace("=", "")
                            .Replace("/", "")
                            .Replace("\\", ""))));
            ...
            lock (cache)
            {
                foreach (var pair in dataInfos)
                {
                    ...
                    if (objType == null)
                    {
                        Debug.WriteLine("Not found in the database: " + type.Name);
                        continue;
                        //throw new Exception("Not found in the database: " + type.Name);
                    }
