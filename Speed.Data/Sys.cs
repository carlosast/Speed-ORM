...
namespace Speed.Data
{
    ...
    public class Sys
    {
        ...
        static Sys()
        {
            var ass = Assembly.GetEntryAssembly() != null ? Assembly.GetEntryAssembly() : Assembly.GetExecutingAssembly();
            string dir = ass.CodeBase;
            if (dir.StartsWith("file:///"))
                dir = dir.Replace("file:///", null);
        ...
        {
            Database db = new Database(providerType, connectionString);
            db.Open();
            return db;
        }
        ...
        public static void RunInThread(bool isBackground, object parameter, Action action)
        {
            var t = new Thread(new ThreadStart(action));
            t.IsBackground = isBackground;
            t.Start(parameter);
